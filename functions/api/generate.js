// POST /api/generate { client_id, brief, cv } -> { pack, provenance, failures, duration_ms }
//
// The one place in this deployment where a model call happens (#6). Architecture §5.6: "Every
// model call goes through one Function, which is also the boundary that lets the data posture
// in §6.4 stay switchable." Do not add a second call site — the switchability is the whole
// reason this is one Function rather than a helper anyone can reach for.
//
// A thin adapter, the same shape as the other four: parse, guard, delegate, serialise.
// Everything that could be wrong about the generation is `src/generate.js`'s problem.
//
...
import { getClient, recordEvent } from "../../src/store.js";
