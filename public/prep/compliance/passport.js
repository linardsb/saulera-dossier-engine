 ...
 *
 * SHOWCASE BRANCH ONLY: the target is /prep/compliance/demo, not the sign-in page. This mirrors
 * what 104ed15 did to the /prep/ junction — a demo should never be handed a code-entry form for
 * a code nobody will receive, and a session that lapses mid-demo should re-open the passport
 * rather than dead-end. public/prep/compliance/login.html is deliberately KEPT: every expiry
 * nudge and every rejection email points at /prep/compliance/login (src/prep/email.js,
 * src/compliance/nudges.js), so the page is still the door those emails open. Only this
 * in-page bounce moves. Restore the line below when the showcase branch is deleted.
 */
function bounce() {
  window.location.replace("/prep/compliance/demo");
  return new Promise(() => {});
}
